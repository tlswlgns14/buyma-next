...
async function extractYouthisyoursProduct(url: URL): Promise<ProductDraft> {
  ...
  const descriptionKo = normalizeTextBlock(
  ...
  const colors = resolveColors(descriptionKo, title);
  const sizeMeasurements = parseSizeMeasurements(descriptionKo);
...
function normalizeMeasurementKey(value: string): { key: string; multiplier?: number } | null {
  ...
  if (key === "HEM" || key === "HEM WIDE" || key === "HEM WIDTH") return { key: "hemwidth" };
